...
public class ClockSettingsFragment extends Fragment {
    ...
    private void setButtons() {
        ...
        btcurrentclockclickapp = (TextView) mView.findViewById(R.id.textViewCurrentClockClickApp);
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
                ...
                if (clockapp != null) {
                    if(clockapp != "NONE") {
                        try {
                            PackageInfo pkgInfo = mContext.getPackageManager().getPackageInfo(clockapp, 0);
                            String pkgName = pkgInfo.packageName;
                            ApplicationInfo appInfo = mContext.getPackageManager().getApplicationInfo(pkgName, 0);
                            String appLabel = mContext.getPackageManager().getApplicationLabel(appInfo).toString();
                            CharSequence appText = "(" + appLabel + " <" + pkgName + ">)";
                            btcurrentclockclickapp.setText(appText);
                            btcurrentclockclickapp.invalidate();
                            Log.d(TAG, "Clock app set: " + pkgName + " -- widgetID: " + appWidgetId);
                            //allClockApps.add(clockImpls[i]);
                        } catch (PackageManager.NameNotFoundException e) {
                            e.printStackTrace();
                            CharSequence appText = "";
                            btcurrentclockclickapp.setText(appText);
                            btcurrentclockclickapp.invalidate();
                        }
                    }else{
                        btcurrentclockclickapp.setText(mContext.getString(R.string.p_Title));
                        btcurrentclockclickapp.invalidate();
                        Log.d(TAG, "Clock app set: " + mContext.getString(R.string.p_Title) + " -- widgetID: " + appWidgetId);
                    }
                }
            }
        });
